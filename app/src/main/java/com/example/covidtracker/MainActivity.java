...
// This is the basic building block of Volley. The name speaks for itself. A RequestQueue is used to queue all the requests and handle the responses.
// Also, it is responsible for managing the worker threads for parallel requests besides reading and writing to and from the cache. A RequestQueue
// works in a FIFO manner(First In First Out)
// . At a time it can handle four requests simultaneously. A RequestQueue is instantiated in the following way in a class.
